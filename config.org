...
(after! evil
  (setq evil-move-cursor-back nil     ; don't move the block cursor when toggling insert mode
        ...
        ;; evil-want-minibuffer t        ; enable `evil-mode' in the minibuffer
...
Make the cursor blue when the theme is gruvbox.
#+begin_src emacs-lisp
(after! evil
  (when (eq doom-theme 'doom-gruvbox)
    ;; this needs to be defined as a constant since evil gets loaded before doom-color
...
Fix nerd fonts being displayed too small.
#+begin_src emacs-lisp
(after! ghostel
  (defun my/ghostel--display-without-height (display)
    "Return DISPLAY without Ghostel's glyph-height clamp."
    (cond
     ((and (listp display)
           (assq 'height display))
      (delq nil
            (mapcar (lambda (spec)
                      (unless (eq (car-safe spec) 'height)
                        spec))
                    display)))
     (t display)))

  (defun my/ghostel--fix-nerd-icon-heights (&rest _)
    "Remove Ghostel display-height clamps from Nerd Font private glyphs."
        ...
        (save-excursion
          ...
          (while (< (point) end)
            (let ((ch (char-after))
              ...
              (when (and ch display
                         (or (and (>= ch #xe000) (<= ch #xf8ff))
                             (and (>= ch #xf0000) (<= ch #xffffd))
                             (and (>= ch #x100000) (<= ch #x10fffd))))
                (let ((new-display (my/ghostel--display-without-height display)))
                  (if new-display
  ...
  (advice-add #'ghostel--delayed-redraw
              :after #'my/ghostel--fix-nerd-icon-heights)
  (add-hook 'ghostel-mode-hook #'my/ghostel--fix-nerd-icon-heights))
#+end_src
...
(after! eglot
  (if (and (featurep :system 'macos) (file-executable-p "/opt/homebrew/opt/llvm/bin/clangd"))
      (add-to-list 'eglot-server-programs
                   '((c-mode c++-mode cc-mode c-ts-mode)
                     . ("/opt/homebrew/opt/llvm/bin/clangd" "-j=3" "--clang-tidy")))))
...
Enable =yaml-ts-mode= in =.clang-format= files.
#+begin_src emacs-lisp
(add-to-list 'auto-mode-alist
             (cons "\\.clang-format\\'"
                   (if (modulep! :lang yaml +tree-sitter)
                       'yaml-ts-mode
                     'yaml-mode)))
#+end_src
*** java
