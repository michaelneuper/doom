...
Some =which-key= adjustments copied from https://tecosaur.github.io/emacs-config/config.html#which-key.
#+begin_src emacs-lisp
...
(after! dirvish
  (setq! dirvish-quick-access-entries
         `(("h" "~/" "Home")
           ...
           ("n" "/scpx:NARGA:" "NARGA")))) ; `scp` ensures large files are transferred out of band via `scp` rather than encoded in the shell session
                                           ; `x` ensures the session is opened with `/bin/sh -i` via `-o RemoteCommand`
...
;; custom functions for inserting nodes
